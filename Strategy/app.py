...
# 初始化所有可能的优化参数 (修复点：添加了 opt_d)
for key in ['opt_short', 'opt_long', 'opt_a', 'opt_b', 'opt_c', 'opt_d']:
...
def plot_chart(df, code, line1, line2, strategy_name, period_tag):
    ...
    if "R-Breaker" in strategy_name:
        fig.add_trace(go.Scatter(x=df.index, y=df['Sbreak'], name='突破卖出线(Sbreak)', line=dict(color='red', dash='dot')))
        fig.add_trace(go.Scatter(x=df.index, y=df['Bbreak'], name='突破买入线(Bbreak)', line=dict(color='green', dash='dot')))
    ...
    fig.update_layout(title=f"{code} {strategy_name} ({period_tag})", height=600, template="plotly_white", hovermode="x unified")
    return fig
...
def main():
    ...
    target_code = st.sidebar.selectbox("选择标的", options=list(ASSET_OPTIONS.keys()), format_func=lambda x: ASSET_OPTIONS[x])
    period_mode = st.sidebar.radio("选择周期", ["日线", "周线"], horizontal=True)
    strategy_type = st.sidebar.radio("选择策略", ["双均线策略 (Double MA)", "自动扶梯策略 (Escalator)", "R-Breaker 策略"])
    ...
    # 显示指标面板
    last = df_res.iloc[-1]
    prev_signal = df_res.iloc[-2]['Signal'] if len(df_res)>1 else 0
    ...
    status_color = "normal" if last['Signal'] == 1 else "inverse"
    cols[1].metric("策略状态", current_status, delta=None)
    
    signal_count = len(df_res[df_res['Position'] != 0])
    cols[2].metric("周期内交易次数", f"{signal_count} 次")
    cols[3].metric("数据最后更新", last.name.strftime('%Y-%m-%d'))
